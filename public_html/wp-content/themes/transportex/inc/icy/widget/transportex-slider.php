...
class transportex_slider_widget extends WP_Widget {    
        ...
        <figure>
            <?php if( !empty($instance['image_uri']) ): ?>
                ...
                <?php else:
                    ...
                    echo '<img src="'.esc_url( get_template_directory_uri() ).'/images/slide/slide.jpg" class="img-fluid" alt="'.apply_filters('widget_title', $instance['name']).'" />'; 
    ...
    function form($instance) { ?>
				...
				<td>
                    <label for="<?php echo $this->get_field_id('slider_title'); ?>"><?php _e('Title', 'transportex'); ?></label>
                </td>
				<td>
				...
				<td>
                    <label for="<?php echo $this->get_field_id('slider_desc'); ?>"><?php _e('Description', 'transportex'); ?></label>
                </td>
				<td>
            ...
            if ( !empty($instance['image_uri']) ) :
                ...
                echo '<img class="custom_media_image_team" src="' . $instance['image_uri'] . '" style="margin:0;padding:0;max-width:100px;float:left;display:inline-block" alt="'.__( 'Upload Image', 'transportex' ).'" /><br />';
        ...
        <table>
			<tr>
                <td style="width: 50%;">
                    <label for="<?php echo $this->get_field_id('btnone'); ?>"><?php _e('Button Text', 'transportex'); ?></label>
                </td>
                <td>
                    <label for="<?php echo $this->get_field_id('btnonelink'); ?>"><?php _e('Button Link', 'transportex'); ?></label>
                </td>
            </tr>
            <tr>
                <td>
            ...
            <tr>
                <td colspan="2">
                    <input type="checkbox" name="<?php echo $this->get_field_name('open_btnone_new_window'); ?>" id="<?php echo $this->get_field_id('open_btnone_new_window'); ?>" <?php if( !empty($instance['open_btnone_new_window']) ): checked( (bool) $instance['open_btnone_new_window'], true ); endif; ?> ><?php _e( 'Open link in a new tab','transportex' ); ?>
                </td>
            </tr>
        </table>
    <?php
